...
library (ape)
HinchChromtree<-read.nexus("./export.nex")
...
setwd("./Nodo 4-3 Hinchliff")
Nodo433<-read.table("Nodo4-3_chrom_Hinchliff - copia.txt", header=F,sep=";") 
Nodo43<-as.character(Nodo433[,1]) #Convierto en caracter, porque es obligatorio para hacer el pruning
# Saco el Nodo 4-3
Nodo43tree<-drop.tip(phy=HinchChromtree, setdiff(HinchChromtree$tip.label,Nodo43))
plot.phylo(Nodo43tree, cex = 0.7,type="fan",no.margin=T)
write.tree(Nodo43tree, file="Nodo43.tree")


#Nodo 4-2-1

setwd("./Nodo 4-2-1 Hinchliff")
Nodo4211<-read.table("Nodo 4-2-1_chrom_Hinchliff - copia.txt", header=F,sep=";")  
Nodo421<-as.character(Nodo4211[,1]) #Convierto en caracter, porque es obligatorio para hacer el pruning
# Saco el Nodo 4-2-1
Nodo421tree<-drop.tip(phy=HinchChromtree, setdiff(HinchChromtree$tip.label,Nodo421))
plot.phylo(Nodo421tree, cex = 0.7,type="fan",no.margin=T)
write.tree(Nodo421tree, file="Nodo421.tree")
